  ...
  <body>
    <div class="container1">
      ...
      <?php
        ...
        $gen = $_POST['gendertype'];


        if ($dept =="all" && $gen=="all") {
          $sql = "SELECT * FROM doctor_list";
        }elseif ($gen=="all") {
          $sql = "SELECT * FROM doctor_list WHERE
                                      department = '$dept'";
        }elseif ($dept =="all") {
          $sql = "SELECT * FROM doctor_list WHERE
                                      gender = '$gen'";
          ...
          $sql = "SELECT * FROM doctor_list WHERE
                                      department = '$dept' AND gender = '$gen'";
        }

        $result = mysqli_query($conn,$sql);              // for table
        $aresult = mysqli_query($conn,$sql);              // for alert


        $adoctors = $aresult->fetch_assoc();       // for alert => get the whole assosiative array
        if (empty($adoctors['id'])){              // compare just the id section of the assosiative array, if empty, no data
          ?>
            <script>
              alert("Cannot find such data !!");
      ...
      <section class="doctor">
        ...
        while($row=$result->fetch_assoc()){   // for table
          $doctors = $row;
        ?>
        <tbody>
